...
prior.z_qsos  = Full_catalog.all_zqso_dr7(prior_ind);
prior.c4_ind = prior_ind;
prior.z_c4 = Full_catalog.all_z_civ_c13(prior_ind);
...
if (ischar(test_ind_dr12))
    test_ind_dr12 = eval(test_ind_dr12);
end

all_wavelengths    =    all_wavelengths(test_ind_dr12);
all_flux           =           all_flux(test_ind_dr12);
all_noise_variance = all_noise_variance(test_ind_dr12);
all_pixel_mask     =     all_pixel_mask(test_ind_dr12);
% all_sigma_pixel    =    all_sigma_pixel(test_ind_dr12);
z_qsos             =           all_zqso_dr12(test_ind_dr12);
...
fAVG= 1/(2*nAVG + 1);
for quasar_ind = ind_start:ind_end
    tic;
...
variables_to_save = {'release', 'training_set_name', ...
     'prior_ind', 'release', ...
    'test_ind_dr12', 'prior_z_qso_increase', ...
    ...
    'map_z_c4L2', 'map_N_c4L2', 'map_sigma_c4L2', 'p_c4', 'ind_start', 'ind_end'};
