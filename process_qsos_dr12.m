...
for quasar_ind = 1:num_quasars
    ...
    this_p_c4(1) = nnz(less_systems(:,1)>0 & less_systems(:,2)==-1)/this_num_quasars;  % P(exactly 1 CIV)
    for i=2:max_civ
        this_p_c4(i) = nnz(less_systems(:,i)>0 & less_systems(:, i+1)==-1)/nnz(less_systems(:,i-1)>0); % P(exactly n CIV | at least n-1 CIV)
        if (this_p_c4(i-1)==0)
        this_p_c4(i) = 0;
        end
    end
    ...
    for i = 1:max_civ
        ...
        % if this_p_c4(i) > 0.5
        %     log_priors_no_c4(quasar_ind, i) = -1e100; % practically -Inf
        % else
        log_priors_no_c4(quasar_ind, i) = log(1 - this_p_c4(i));
        % end
   ...
   % by adding the mean of ith and ith +1 element
    ...
    % for num_c4=1:1
        ...
        % fprintf('size(d)=%d-%d, size(M)=%d-%d\n',...
        % size(this_noise_variance(ind_not_remove)),...
        ...
        parfor i = 1:num_C4_samples
            ...
            % fprintf('%.2e-%.2e\n', min(absorptionL2_fine), max(absorptionL2_fine))
            % fprintf('%.2e-%.2e\n', min(absorptionL2), max(absorptionL2))
            % fprintf('%.2e-%.2e\n', min(c4_muL2), max(c4_muL2))
            % fprintf('%.2e-%.2e\n', min(c4_ML2), max(c4_ML2))
        ...
        if(plotting==1)
            ...
            if voigtType == 0
                absorptionL1_fine= voigt0(finer(padded_wavelengths, nAVG),...
                            map_z_c4L1(quasar_ind, num_c4), 10^map_N_c4L1(quasar_ind, num_c4),...
                            num_lines, map_sigma_c4L1(quasar_ind, num_c4));
            end
            ...
            if voigtType == 0
                absorptionL2_fine= voigt0(finer(padded_wavelengths, nAVG),...
                    map_z_c4L2(quasar_ind,  num_c4), 10^map_N_c4L2(quasar_ind, num_c4),...
                    num_lines, map_sigma_c4L2(quasar_ind, num_c4));
            end
            ...
            EW(quasar_ind, num_c4) = trapz(this_wavelengths, 1-absorptionL2);
