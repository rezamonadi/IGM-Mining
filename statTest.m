...
ind_low_numc4 = zeros([num_quasars,1]);
ind_high = zeros([num_quasars,1]);
ind_high_numc4 = zeros([num_quasars,1]);
ind_0 = zeros([num_quasars,1]);
ind_0_numc4 = zeros([num_quasars,1]);

ind_dv_low = zeros([num_quasars,1]);
ind_dv_low_numc4 = zeros([num_quasars,1]);
ind_dv_high = zeros([num_quasars,1]);
ind_dv_high_numc4 = zeros([num_quasars,1]);
ind_dv_mid = zeros([num_quasars,1]);
ind_dv_mid_numc4 = zeros([num_quasars,1]);
...
for tr=0.95
    ...
    PM1GP1highREW = 0; 
    lw =5;
    dNdZ_PM0GP1 = nan([num_quasars, max_civ]);
    N_c4_PM1GP1 = nan([num_quasars, max_civ]);
    s_c4_PM1GP1 = nan([num_quasars, max_civ]);
    N_c4_PM1GP1 = nan([num_quasars, max_civ]);
    s_c4_PM0GP1 = nan([num_quasars, max_civ]);
    N_c4_GP1 = nan([num_quasars, max_civ]);
    s_c4_GP1 = nan([num_quasars, max_civ]);
    REW_1548_dr7_PM1GP1_PM = nan([num_quasars, max_civ]);
    REW_1548_dr7_PM1GP1_PMerr = nan([num_quasars, max_civ]);
    REW_1548_dr7_PM1GP1_GPflux = nan([num_quasars, max_civ]);
    REW_1548_dr7_PM1GP1_GPfluxErr = nan([num_quasars, max_civ]);
    REW_1548_dr7_PM1GP1_GPVoigt = nan([num_quasars, max_civ]);
    REW_1550_dr7_PM1GP1_PM = nan([num_quasars, max_civ]);
    REW_1550_dr7_PM1GP1_PMerr = nan([num_quasars, max_civ]);
    REW_1550_dr7_PM1GP1_GPflux = nan([num_quasars, max_civ]);
    REW_1550_dr7_PM1GP1_GPfluxErr = nan([num_quasars, max_civ]);
    REW_1550_dr7_PM1GP1_GPVoigt = nan([num_quasars, max_civ]);
    map_sigma_c4L2_PM1GP1 = nan([num_quasars,max_civ]);
    map_N_c4L2_PM1GP1 = nan([num_quasars,max_civ]);
    REW_1548_dr7_PM0GP1 = nan([num_quasars, max_civ]);
    REW_1548_dr7_PM1GP0 = nan([num_quasars, 17]);
    DN = nan(num_quasars,max_civ);
    rdz = nan(num_quasars,max_civ);
    Dv_PM1GP1 = nan([num_quasars, max_civ]);
    indPM1GP1_in_GP = nan(num_quasars,max_civ);
    indPM0GP1_in_GP = nan(num_quasars,max_civ);
    indPM1GP0_in_GP = nan(num_quasars,max_civ);
    diffEW1548 = nan([num_quasars,max_civ]);
    diffEW1550 = nan([num_quasars,max_civ]);
    zciv_PM1GP1_PM = nan([num_quasars, max_civ]);
    nn=0;mm=0;
    ...
    for quasar_ind=1:num_quasars
        ...
        for j=1:max_civ
            if (p_c4(quasar_ind, j)>=tr)
                GP = GP+1;
                N_c4_GP1(quasar_ind, j) = map_N_c4L2(quasar_ind, j);
%                 s_c4_GP1(quasar_ind, j) = map_sigma_c4L2(quasar_ind, j);
            end
        end
        thisTP=0;
        for i=1:17
            ...
            if(Z_C13_1(quasar_ind, i)>0)
                ...
                DZ = Z_C13_1(quasar_ind, i) - map_z_c4L2(quasar_ind, :);
                [DZ_min_abs, indMin] = min(abs(DZ));
                DZ_min = DZ(indMin);
                
                
                    j = indMin;
                    if (p_c4(quasar_ind, j)>=tr)  % PM1GP1
    ...
    %                         s_c4_PM1GP1(quasar_ind, j) = map_sigma_c4L2(quasar_ind, j);
                            % 1548
                            REW_1548_dr7_PM1GP1_GPVoigt(quasar_ind, j) = REW_1548_DR7_voigt(quasar_ind, j);
                            REW_1548_dr7_PM1GP1_GPflux(quasar_ind, j) = REW_1548_DR7_flux(quasar_ind, j);
                            REW_1548_dr7_PM1GP1_PM(quasar_ind, j) = EW_C13_1548(quasar_ind, i);
                            REW_1548_dr7_PM1GP1_PMerr(quasar_ind, j) = errEW_C13_1548(quasar_ind,i);
                            REW_1548_dr7_PM1GP1_GPfluxErr(quasar_ind, j) = ErrREW_1548_flux(quasar_ind,j);
                            map_sigma_c4L2_PM1GP1(quasar_ind,j) = map_sigma_c4L2(quasar_ind,j);
                            map_N_c4L2_PM1GP1(quasar_ind, j) = map_N_c4L2(quasar_ind, j);
                            zciv_PM1GP1_PM(quasar_ind, j) = Z_C13_1(quasar_ind, i);
                            ...
                            DN(quasar_ind, j) = map_N_c4L2(quasar_ind, j) - all_N_civ(quasar_ind,i);

                            errMax = max(REW_1548_dr7_PM1GP1_PMerr(quasar_ind, j), REW_1548_dr7_PM1GP1_GPfluxErr(quasar_ind, j));
                            diffEW1548(quasar_ind,j) = (REW_1548_dr7_PM1GP1_GPflux(quasar_ind, j) - REW_1548_dr7_PM1GP1_PM(quasar_ind, j))/errMax;
                            if diffEW1548(quasar_ind,j)>2
                                    ind_high(quasar_ind) = 1;
                                ind_high_numc4(quasar_ind)=j;
                            end
                            if diffEW1548(quasar_ind,j)<-2
                                ind_low(quasar_ind) = 1;
                                ind_low_numc4(quasar_ind)=j;
                            end
                            if abs(diffEW1548(quasar_ind,j))<0.02
                                ind_0(quasar_ind) = 1;
                                ind_0_numc4(quasar_ind) = j;
                            end

                            % 1550
                            REW_1550_dr7_PM1GP1_GPVoigt(quasar_ind, j) = REW_1550_DR7_voigt(quasar_ind, j);
                            REW_1550_dr7_PM1GP1_GPflux(quasar_ind, j) = REW_1550_DR7_flux(quasar_ind, j);
                            REW_1550_dr7_PM1GP1_PM(quasar_ind, j) = EW_C13_1550(quasar_ind, i);
                            REW_1550_dr7_PM1GP1_PMerr(quasar_ind, j) = errEW_C13_1550(quasar_ind,i);
                            REW_1550_dr7_PM1GP1_GPfluxErr(quasar_ind, j) = ErrREW_1550_flux(quasar_ind,j);
                            errTotal = sqrt(REW_1550_dr7_PM1GP1_PMerr(quasar_ind, j)^2 +  REW_1550_dr7_PM1GP1_GPfluxErr(quasar_ind, j)^2);
                            diffEW1550(quasar_ind,j) = (REW_1550_dr7_PM1GP1_GPflux(quasar_ind, j) - REW_1550_dr7_PM1GP1_PM(quasar_ind, j))/errTotal;
                            ...
                            if (Dv_PM1GP1(quasar_ind, j)>50)
                                ...
                                ind_dv_high_numc4(quasar_ind) = j;
                            end
                            if (Dv_PM1GP1(quasar_ind, j)<-150)
                                ind_dv_low(quasar_ind) =1;
                                ind_dv_low_numc4(quasar_ind)=j;
                            end
                            if (Dv_PM1GP1(quasar_ind, j)>-50) & (Dv_PM1GP1(quasar_ind, j)<-48)
                                ind_dv_mid(quasar_ind) =1;
                                ind_dv_mid_numc4(quasar_ind)=j;
                            end

                            if REW_1548_dr7_PM1GP1_GPflux(quasar_ind, i)> 1.2
    ...
    %                         REW_1548_dr7_PM1GP0(quasar_ind, i) = EW_C13_1548(quasar_ind, i);
    %                         if REW_1548_dr7_PM1GP0(quasar_ind, i)> 1.2
...
%                     s_c4_PM0GP1(quasar_ind, j) = map_sigma_c4L2(quasar_ind, j);
                    REW_1548_dr7_PM0GP1(quasar_ind, j) = REW_1548_DR7_flux(quasar_ind, j);
                    mm = mm+1;
...
% h = histogram(reshape(REW_1548_dr7_PM1GP1_PM(~isnan(REW_1548_dr7_PM1GP1_PM)),[],1), 'normalization', 'pdf');
% h.BinWidth =0.25;
% x3 = h.BinEdges;
...
% h = histogram(reshape(REW_1548_dr7_PM1GP0(~isnan(REW_1548_dr7_PM1GP0)),[],1), 'normalization', 'pdf');
% h.BinWidth =0.25;
% x1 = h.BinEdges;
...
% h = histogram(reshape(REW_1548_dr7_PM0GP1(~isnan(REW_1548_dr7_PM0GP1) & REW_1548_dr7_PM0GP1>0),[],1), 'normalization', 'pdf');
% h.BinWidth=0.25;
% x2 = h.BinEdges;
...
% fig = figure()
% y = reshape(REW_1548_dr7_PM1GP1_GPVoigt(:,:), [], 1); % W_r,GP,voigt
% x = reshape(REW_1548_dr7_PM1GP1_PM(:,:), [], 1); % W_r,PM 
% e = reshape(REW_1548_dr7_PM1GP1_PMerr(:,:,3), [], 1); % err from PM
...
% fig = figure()
% dW_r  = REW_1548_dr7_PM1GP1(:,:,1) - REW_1548_dr7_PM1GP1(:,:,2); % W_r,GP - W_r,PM
...
% fig =  figure();
% y = dW_r./REW_1548_dr7_PM1GP1(:,:,3);
% y = reshape(y,[],1);
% x = REW_1548_dr7_PM1GP1(:,:,2);
% x = reshape(x,[],1);
...
% % Scatter plot of W_r,GP vs. W_r,PM
% fig = figure()
% dW_r_flux  = REW_1548_dr7_PM1GP1(:,:,4) - REW_1548_dr7_PM1GP1(:,:,2); % W_r,GP - W_r,PM
% y = reshape(dW_r, [], 1); % dW_r,PM,GP
% x = reshape(Dv_PM_GP3, [], 1);
...
% % scatter of dW_r/err vs. W_r,PM
% fig =  figure();
% y = dW_r_flux./REW_1548_dr7_PM1GP1(:,:,3);
% y = reshape(y,[],1);
% x = REW_1548_dr7_PM1GP1(:,:,2);
% x = reshape(x,[],1);
...
% %  Boxcar GP vs PM
% fig = figure()
% x = reshape(REW_1548_dr7_PM1GP1_GPflux(:,:), [], 1); % W_r,GP, flux 
% xe = reshape(REW_1548_dr7_PM1GP1_GPfluxErr(:,:), [], 1); % W_r,GP, flux error
% y = reshape(REW_1548_dr7_PM1GP1_PM(:,:), [], 1); % W_r,PM
% ye = reshape(REW_1548_dr7_PM1GP1_PMerr(:,:),[],1); % error(W_r,PM)
% s = scatter(x,y, 10, 'filled')
% s.MarkerFaceAlpha =0.6;
...
% % Voigt vs Boxcar GP
% fig = figure()
% x = reshape(REW_1548_dr7_PM1GP1_GPVoigt(:,:), [], 1); % W_r,GP, voigt
% y = reshape(REW_1548_dr7_PM1GP1_GPflux(:,:), [], 1); % W_r,GP, flux
% ye = reshape(REW_1548_dr7_PM1GP1_GPfluxErr(:,:),[],1); % error(W_r,GP,flux)
% % s = scatter(x,y, 10, 'filled')
% % s.MarkerFaceAlpha =0.6;
...
% difEW1548 = y-x;
% ye = reshape(REW_1548_dr7_PM1GP1_PMerr(:,:),[],1); % error(W_r,PM)
% xe = reshape(REW_1548_dr7_PM1GP1_GPfluxErr(:,:), [], 1); %error() W_r,GP, flux)
% errTotal = sqrt(xe.^2 + ye.^2);
% yPlot = difEW1548./errTotal; % W_r,PM - W_r,GP,flux / errTotal
...
% % scatter  (EW(GP,Flux) - EW(PM))/total(err) vs EW(PM) for 1550A
% fig = figure()
% x = reshape(REW_1550_dr7_PM1GP1_PM(:,:), [], 1); % W_r,PM
% yPlot = reshape(diffEW1550,[],1); % W_r,PM - W_r,GP,flux / errTotal
% c = reshape(map_sigma_c4L2_PM1GP1,[],1);
% s = scatter(x,yPlot,  10, c, 'filled');
% s.MarkerFaceAlpha =0.6;
% cb = colorbar;
...
% xlim([min(x)-0.05, max(x)+.05])
% ylim([min(yPlot)-0.05, max(yPlot)+.05])
% xlabel('${\rm W}_{r,1550}^{\rm PM}$~(\AA)', 'interpreter', 'latex')
...
vs = {'ind_low', 'ind_low_numc4', 'ind_high', 'ind_high_numc4', 'ind_0', 'ind_0_numc4', 'diffEW1548', 'diffEW1550', ...
        'ind_dv_low', 'ind_dv_low_numc4', 'ind_dv_high', 'ind_dv_high_numc4', 'ind_dv_mid', 'ind_dv_mid_numc4', 'Dv_PM1GP1',...
        'ind_EW_large_PM1GP0', 'ind_EW_large_PM1GP0_numc4', 'DV_EW_large_PM1GP0', 'ind_PM1GP0'};
save('ind_err.mat', vs{:})
