...
% Please select the the release for loading spectra to test or train
release='dr7';
rl = 12;
...
min_lambda         =  1230;                   % range of rest wavelengths to       Å
max_lambda         = 1555;                    %   model
...
h                  = 15;                       % masking par to remove CIV region 
nAVG               = 0;                       % number of points added between two 
                                             % observed wavelengths to make the Voigt finer
SkyLine            = 1;                       % Handel for removing/not removing sky lines as indicated in C13                                             
Rsl =0;                                      % Handle for including(1) or not including (0) pixel siga in voigt optimization parameters
...
% a                        = 1 ;                         % The belending constant for p_c4_L1
f_L1_nciv                = 0.01;
f_L1_sigma               = 0.05;
min_sigma                = 10e5;                   % cm/s -> b/sqrt(2) -> min Doppler par from Cooksey
max_sigma                = 50e5;                   % cm/s -> b/sqrt(2) -> max Doppler par from Cooksey
...
%         min_z_cut);
min_z_c4 = @(wavelengths) ...         % determines minimum z_civ to search
