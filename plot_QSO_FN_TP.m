...
for quasar_ind=1000:nqso
    ...
    if(num_systems>0 & p_c4s(quasar_ind)>0.90) % TP
        % if(num_systems==0 & p_c4s(quasar_ind)<0.2 & filter_flags(quasar_ind)==0) % FN
        count=count+1;
        fprintf('TP:%d\n',count);
...
% construct dla_mu_map
        ...
        matched_Z_c4= this_Zs(abs(this_Zs -map_z_c4s(quasar_ind))==min(abs(this_Zs-map_z_c4s(quasar_ind))));
        matched_N_c4= this_c4s(abs(this_Zs -map_z_c4s(quasar_ind))==min(abs(this_Zs-map_z_c4s(quasar_ind))));
        ...
        %              min(this_unmasked_wavelengths(TF)))/(1+z_qsos(quasar_ind)));
         ...
         tit = sprintf('Z_{qso}=%.2f, sigma_{MAP}:%.2fkm/s, P(CIV):%.2f\nZ^{CIV}_{MAP}:%.2f, Z^{CIV}_{C13}:%.2f, N_{MAP}:%.2f, N^{CIV}_{C13}:%.2f, ',...
                     z_qsos(quasar_ind), ...
                     map_sigma_c4(quasar_ind,1)/1e5, p_c4s(quasar_ind),...
                     map_z_c4(quasar_ind,1), matched_Z_c4, map_N_c4(quasar_ind,1), matched_N_c4);
        ...
        subplot(2,1,2);
        hold on
        norm_sample_log_likelihoods = sample_log_likelihoods_c4(quasar_ind, :);
        ...
        fid = sprintf('%s/TP-ID-%s.png',  dir,  ID{quasar_ind});
