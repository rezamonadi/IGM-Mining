...
j0=0;
for quasar_ind = [71,148]
    % if z_PM_test(quasar_ind,1)==-1
    %     continue
    % end

    tic;
    z_qso = z_qsos(missed_abs(quasar_ind,2));
    fprintf('processing quasar %i/%i (z_QSO = %0.4f) ...', ...
                              quasar_ind, num_quasars, z_qso);
    
    this_wavelengths    =    all_wavelengths{(missed_abs(quasar_ind,2))};
    this_wavelengths    =              this_wavelengths';
    this_flux           =           all_flux{missed_abs(quasar_ind,2)}; 
    this_flux           =                     this_flux';
    this_noise_variance = all_noise_variance{missed_abs(quasar_ind,2)};
    this_noise_variance =           this_noise_variance';
    this_pixel_mask     =     all_pixel_mask{missed_abs(quasar_ind,2)};
    this_pixel_mask     =               this_pixel_mask';
    this_sigma_pixel    =     all_sigma_pixel{missed_abs(quasar_ind,2)};
    ...
    unmasked_ind = (this_rest_wavelengths >= min_lambda) & ...
       (this_rest_wavelengths <= max_lambda) & (this_sigma_pixel>0) &...
         ~(abs(this_rest_wavelengths- 1549.48)< 25); %& ... % CIV emission line masking --> 30A comes from typical FWHM of CIV emission in SDSS Cite{Monadi & Bird-2022}
  ...
  % c4 existence prior
    ...
    for i = 1:max_MgII
        fprintf(' ...     p(%i  MgIIs | z_QSO)       : %0.3f\n', i, this_p_MgII(i));
            log_priors_no_MgII(missed_abs(quasar_ind,2), i) = ...
                log(1 - this_p_MgII(i));
        fprintf(' ...     p(no MgII  | z_QSO)       : %0.3f\n', exp(log_priors_no_MgII(missed_abs(quasar_ind,2), i)) );
    end

    log_priors_MgII(missed_abs(quasar_ind,2),:) = log(this_p_MgII(:));
    ...
    min_z_MgIIs(missed_abs(quasar_ind,2)) = min_z_MgII(this_wavelengths, z_qso);
    % instead of this_wavelengths I puting 1310A where is the lower limit of CIV search in C13
    %min_z_MgIIs(missed_abs(quasar_ind,2)) = min_z_MgII(1310, z_qso);
    max_z_MgIIs(missed_abs(quasar_ind,2)) = max_z_MgII(z_qso, max_z_cut);

    sample_z_MgII = ...
        min_z_MgIIs(missed_abs(quasar_ind,2)) +  ...
        (max_z_MgIIs(missed_abs(quasar_ind,2)) - min_z_MgIIs(missed_abs(quasar_ind,2))) * offset_z_samples;
   ...
   % by adding the mean of ith and ith +1 element
    ...
    for num_MgII=1:max_MgII
        ...
        if(num_MgII>1)
            if((p_MgII(missed_abs(quasar_ind,2), num_MgII-1)>p_MgIIL1(missed_abs(quasar_ind,2), num_MgII-1)) & ...
                (p_MgII(missed_abs(quasar_ind,2), num_MgII-1)>p_no_MgII(missed_abs(quasar_ind,2), num_MgII-1)))
                ind_not_remove = ind_not_remove  & ...
                    (abs(this_z_2796 - map_z_MgIIL2(missed_abs(quasar_ind,2), num_MgII-1))>kms_to_z(dv_mask)*(1+map_z_MgIIL2(missed_abs(quasar_ind,2), num_MgII-1))) & ...
                    (abs(this_z_2803 - map_z_MgIIL2(missed_abs(quasar_ind,2), num_MgII-1))>kms_to_z(dv_mask)*(1+map_z_MgIIL2(missed_abs(quasar_ind,2), num_MgII-1)));
            end

            if((p_MgIIL1(missed_abs(quasar_ind,2), num_MgII-1)>p_MgII(missed_abs(quasar_ind,2), num_MgII-1)) & ...
                        (p_MgIIL1(missed_abs(quasar_ind,2), num_MgII-1)>p_no_MgII(missed_abs(quasar_ind,2), num_MgII-1)))
                ind_not_remove = ind_not_remove  & ...
                (abs(this_z_2796 - map_z_MgIIL1(missed_abs(quasar_ind,2), num_MgII-1))>kms_to_z(dv_mask)*(1+map_z_MgIIL1(missed_abs(quasar_ind,2), num_MgII-1)));
            end

            if((p_no_MgII(missed_abs(quasar_ind,2), num_MgII-1)>=p_MgII(missed_abs(quasar_ind,2), num_MgII-1)) & ...
                (p_no_MgII(missed_abs(quasar_ind,2), num_MgII-1)>=p_MgIIL1(missed_abs(quasar_ind,2), num_MgII-1)))
        ...
        log_likelihoods_no_MgII(missed_abs(quasar_ind,2), num_MgII) = ...
        ...
        log_posteriors_no_MgII(missed_abs(quasar_ind,2), num_MgII) = ...
            log_priors_no_MgII(missed_abs(quasar_ind,2), num_MgII) + log_likelihoods_no_MgII(missed_abs(quasar_ind,2), num_MgII);
        ...
        for i = 1:num_MgII_samples
            ...
            sample_log_likelihoods_MgIIL2(missed_abs(quasar_ind,2), i, num_MgII) = ...
            ...
            sample_log_likelihoods_MgIIL1(missed_abs(quasar_ind,2), i) = ...
            log_mvnpdf_low_rank(this_flux(ind_not_remove),...
        ...
        max_log_likelihoodL1 = max(sample_log_likelihoods_MgIIL1(missed_abs(quasar_ind,2), :));
        sample_probabilitiesL1 = ...
            exp(sample_log_likelihoods_MgIIL1(missed_abs(quasar_ind,2), :) - ...
            max_log_likelihoodL1);
        log_likelihoods_MgIIL1(missed_abs(quasar_ind,2), num_MgII) = ...
            max_log_likelihoodL1 + log(mean(sample_probabilitiesL1));% ...
            % - log(num_MgII_samples)*(num_MgII-1);

        log_posteriors_MgIIL1(missed_abs(quasar_ind,2), num_MgII) = ...
        log_priors_MgII(missed_abs(quasar_ind,2), num_MgII) + log_likelihoods_MgIIL1(missed_abs(quasar_ind,2), num_MgII);
        ...
        max_log_likelihoodL2 = max(sample_log_likelihoods_MgIIL2(missed_abs(quasar_ind,2), :, num_MgII));
        sample_probabilitiesL2 = ...
            exp(sample_log_likelihoods_MgIIL2(missed_abs(quasar_ind,2), :, num_MgII)  ... 
            - max_log_likelihoodL2);
        log_likelihoods_MgIIL2(missed_abs(quasar_ind,2), num_MgII) = ...
            max_log_likelihoodL2 + log(mean(sample_probabilitiesL2));%...
            % - log(num_MgII_samples)*(num_MgII-1);


        log_posteriors_MgIIL2(missed_abs(quasar_ind,2), num_MgII) = ...
            log_priors_MgII(missed_abs(quasar_ind,2), num_MgII) + log_likelihoods_MgIIL2(missed_abs(quasar_ind,2), num_MgII);

        fprintf(' ... log p(D | z_QSO,    CIV)     : %0.2f\n', ...
            log_likelihoods_MgIIL2(missed_abs(quasar_ind,2), num_MgII));
        fprintf(' ... log p(CIV | D, z_QSO)        : %0.2f\n', ...
            log_posteriors_MgIIL2(missed_abs(quasar_ind,2), num_MgII));
        [~, maxindL1] = max(sample_log_likelihoods_MgIIL1(missed_abs(quasar_ind,2), :));
        map_z_MgIIL1(missed_abs(quasar_ind,2), num_MgII )    = sample_z_MgII(maxindL1);        
        map_N_MgIIL1(missed_abs(quasar_ind,2), num_MgII)  = log_nMgII_samples(maxindL1);
        map_sigma_MgIIL1(missed_abs(quasar_ind,2), num_MgII)  = sigma_MgII_samples(maxindL1);
        % fprintf('L1\nmap(N): %.2f, map(z_MgII): %.2f, map(b/1e5): %.2f\n',map_N_MgIIL1(missed_abs(quasar_ind,2), num_MgII),...
            % map_z_MgIIL1(missed_abs(quasar_ind,2), num_MgII), map_sigma_MgIIL1(missed_abs(quasar_ind,2), num_MgII)/1e5);



        [~, maxindL2] = max(sample_log_likelihoods_MgIIL2(missed_abs(quasar_ind,2), :, num_MgII));
        map_z_MgIIL2(missed_abs(quasar_ind,2), num_MgII)    = sample_z_MgII(maxindL2);        
        map_N_MgIIL2(missed_abs(quasar_ind,2), num_MgII)  = log_nMgII_samples(maxindL2);
        map_sigma_MgIIL2(missed_abs(quasar_ind,2), num_MgII)  = sigma_MgII_samples(maxindL2);
        % fprintf('L2\nmap(N): %.2f, map(z_MgII): %.2f, map(b/1e5): %.2f\n',...
        % map_N_MgIIL2(missed_abs(quasar_ind,2), num_MgII), map_z_MgIIL2(missed_abs(quasar_ind,2), num_MgII),...
        % map_sigma_MgIIL2(missed_abs(quasar_ind,2), num_MgII)/1e5);

        max_log_posteriors = max([log_posteriors_no_MgII(missed_abs(quasar_ind,2), num_MgII), log_posteriors_MgIIL1(missed_abs(quasar_ind,2), num_MgII), log_posteriors_MgIIL2(missed_abs(quasar_ind,2),num_MgII)], [], 2);

        model_posteriors = ...
                exp(bsxfun(@minus, ...           
                [log_posteriors_no_MgII(missed_abs(quasar_ind,2), num_MgII), log_posteriors_MgIIL1(missed_abs(quasar_ind,2), num_MgII), log_posteriors_MgIIL2(missed_abs(quasar_ind,2), num_MgII)], ...
                max_log_posteriors));
        model_posteriors = ...
        bsxfun(@times, model_posteriors, 1 ./ sum(model_posteriors, 2));

        p_no_MgII(missed_abs(quasar_ind,2), num_MgII) = model_posteriors(1);
        p_MgIIL1(missed_abs(quasar_ind,2), num_MgII)  = model_posteriors(2);
        p_MgII(missed_abs(quasar_ind,2), num_MgII)    = 1 - p_no_MgII(missed_abs(quasar_ind,2), num_MgII) -...
                                    p_MgIIL1(missed_abs(quasar_ind,2), num_MgII);

        c4_pixel_ind1 = abs(this_wavelengths - (1+map_z_MgIIL2(missed_abs(quasar_ind,2), num_MgII))*mgii_2796_wavelength)<3;
        c4_pixel_ind2 = abs(this_wavelengths - (1+map_z_MgIIL2(missed_abs(quasar_ind,2), num_MgII))*mgii_2803_wavelength)<3;
        num_pixel_MgII(missed_abs(quasar_ind,2), num_MgII, 1) = nnz(c4_pixel_ind1);
        num_pixel_MgII(missed_abs(quasar_ind,2), num_MgII, 2) = nnz(c4_pixel_ind2);
        fprintf('CIV pixels:[%d, %d]\n', num_pixel_MgII(missed_abs(quasar_ind,2), num_MgII, :)); 
        ...
        % fprintf('s(emitted_finer_unmasked)-%d-%d\n', size(emitted_wavelengths(finer(this_unmasked_wavelengths, nAVG), z_qso)))                                    
             ...
             aL1_fine = voigt_iP(padded_wavelengths_fine,...
                                         map_z_MgIIL2(missed_abs(quasar_ind,2), num_MgII), ...
                                         10^map_N_MgIIL2(missed_abs(quasar_ind,2), num_MgII), 1,...
                                         map_sigma_MgIIL2(missed_abs(quasar_ind,2), num_MgII), ...
                                         padded_sigma_pixels_fine);

            aL1 = Averager(aL1_fine, nAVG, lenW_unmasked);

            REW_2796_dr7(missed_abs(quasar_ind,2), num_MgII) = trapz(this_unmasked_wavelengths, 1-aL1)/(1+z_qso);
%          
            fprintf('REW(%d,%d)=%e\n', missed_abs(quasar_ind,2), num_MgII, REW_2796_dr7(missed_abs(quasar_ind,2), num_MgII));
%         end

         if(plotting==1) 
            % plotting

            this_ID = ID{missed_abs(quasar_ind,2)};
            max_log_posteriors = max([log_posteriors_no_MgII(missed_abs(quasar_ind,2), num_MgII), log_posteriors_MgIIL1(missed_abs(quasar_ind,2), num_MgII), log_posteriors_MgIIL2(missed_abs(quasar_ind,2),num_MgII)], [], 2);
            num_lines=1;
            absorptionL1_fine= voigt_iP(padded_wavelengths_fine,...
                            map_z_MgIIL1(missed_abs(quasar_ind,2), num_MgII),1.2*(10^map_N_MgIIL1(missed_abs(quasar_ind,2), num_MgII)),...
                            num_lines, map_sigma_MgIIL2(missed_abs(quasar_ind,2), num_MgII), padded_sigma_pixels_fine);
            ...
            absorptionL2_fine= voigt_iP(padded_wavelengths_fine,...
                map_z_MgIIL2(missed_abs(quasar_ind,2),  num_MgII), 1.2*(10^map_N_MgIIL2(missed_abs(quasar_ind,2), num_MgII)),...
                num_lines, map_sigma_MgIIL2(missed_abs(quasar_ind,2), num_MgII), padded_sigma_pixels_fine);
            absorptionL2 = Averager(absorptionL2_fine, nAVG, lenW_unmasked);
            absorptionL2 = absorptionL2(ind);
            ...
            % if (num_MgII==ind_EW_large_PM1GP0_numc4(missed_abs(quasar_ind,2)))




                % ttl = sprintf('ID:%s, zQSO:%.2f, P(CIV)=%.2f, P(S)=%.2f, z_{CIV}=%.6f\nz_{PM}=[%.4f,%.4f,%.4f,%.4f]\nREW_{PM}=[%.3f,%.3f,%.3f,%.3f]\n errREW_{PM}=[%.3f,%.3f,%.3f,%.3f], REW(GP)=%.3f, err(GP)=%.3f',  ...
                %     this_ID, z_qso, p_MgII(missed_abs(quasar_ind,2), num_MgII), p_MgIIL1(missed_abs(quasar_ind,2), num_MgII),  map_z_MgIIL2(missed_abs(quasar_ind,2), num_MgII), ...
                %     z_PM_test(missed_abs(quasar_ind,2),1:4),...
                %     REW_PM(missed_abs(quasar_ind,2),1:4), errREW_PM(missed_abs(quasar_ind,2),1:4),...
                %     REW_2796_dr7_flux(missed_abs(quasar_ind,2), num_MgII), ErrREW_1548_flux(missed_abs(quasar_ind,2), num_MgII));
                DZ = abs(z_PM_test(missed_abs(quasar_ind,2), 1:4) - map_z_MgIIL2(missed_abs(quasar_ind,2),num_MgII));

                dv = DZ./(1+z_PM_test(missed_abs(quasar_ind,2), 1:4))*speed_of_light/1e3;

                ttl = sprintf('ID:%s, zQSO:%.2f, P(MgII)=%.2f, P(S)=%.2f, z_{MgII}=%.6f\nz_{PM}=[%.4f,%.4f,%.4f,%.4f], dv = [%.0f,%.0f, %.0f, %.0f]',  ...
                    this_ID, z_qso, p_MgII(missed_abs(quasar_ind,2), num_MgII), p_MgIIL1(missed_abs(quasar_ind,2), num_MgII),  map_z_MgIIL2(missed_abs(quasar_ind,2), num_MgII), ...
                    z_PM_test(missed_abs(quasar_ind,2),1:4), dv);    
                ttl

                dz_Doppler = kms_to_z(sqrt(2)*4*map_sigma_MgIIL2(missed_abs(quasar_ind,2), num_MgII)/1e5); % in km to z
                dz_mid = (mgii_2803_wavelength - mgii_2796_wavelength)*0.5/mgii_2796_wavelength; % cm/s
                z_EWhigh = min(map_z_MgIIL2(missed_abs(quasar_ind,2), num_MgII) + dz_Doppler*(1+z_qso), map_z_MgIIL2(missed_abs(quasar_ind,2), num_MgII) + dz_mid*(1+z_qso));
                z_EWlow = map_z_MgIIL2(missed_abs(quasar_ind,2), num_MgII) - dz_Doppler*(1+z_qso); 

                z_PM_test_plot = z_PM_test(missed_abs(quasar_ind,2),:);
                z_PM_test_plot = z_PM_test_plot(z_PM_test_plot>0);
                fid = sprintf('flux/ind-%d-MgII-FinalTest-%d.png',  missed_abs(quasar_ind,2), num_MgII);
                ind_zoomL2 = (abs(this_z_2796-map_z_MgIIL2(missed_abs(quasar_ind,2), num_MgII))<20*kms_to_z(map_sigma_MgIIL2(missed_abs(quasar_ind,2), num_MgII)/1e5)*(1+z_qso));
                ind_zoomL1 = (abs(this_z_2796-map_z_MgIIL1(missed_abs(quasar_ind,2), num_MgII))<20*kms_to_z(map_sigma_MgIIL2(missed_abs(quasar_ind,2), num_MgII)/1e5)*(1+z_qso));
         ...
         % end
            ...
            % function y=plt_mu(this_flux, this_wavelengths, mu, missed_abs, quasar_ind, z_qso, M, ttl, fid)

            %plt_mu(this_flux, this_wavelengths, missed_abs, quasar_ind, mu, z_qso, M, ttl, fid)
            ttl
        end
...
filename = sprintf('%s/processed_qsos_tst_1_Masking_CIV-25A%s.mat', ...
