...
line_mask = (rest_wavelengths > 1200 & rest_wavelengths < 1220) | ... % Lyα
             (rest_wavelengths > 1540 & rest_wavelengths < 1560) | ... % C IV
             (rest_wavelengths > 2700 & rest_wavelengths < 2900);    % Mg II and Fe II
...
% PCA initialization with robust component handling
[coefficients, ~, latent] = pca(pca_centered_rest_flux, 'rows', 'complete');
k_actual = min(k, size(coefficients, 2));
initial_M = bsxfun(@times, coefficients(:, 1:k_actual), sqrt(latent(1:k_actual))');

if k_actual < k
    warning('PCA returned only %d components (requested %d). Adjusting model.', k_actual, k);
end
...
ind = (1:(num_rest_pixels * k_actual));
M = reshape(x(ind), [num_rest_pixels, k_actual]);
