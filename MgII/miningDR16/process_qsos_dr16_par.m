...
% Multi Singlet -> no
%
% removing MgII found region after each run 
...
% MgII and useful for training null model (a model without MgII absorption line)
% prior_ind -> those LOSs with MgII absorption and in the half part of test
% test_ind -> second half without any filter flag for testing null and absorption model
% on the LOSs that we know have MgII or not. So, we asses our algorithm in this way.

if (ischar(prior_ind))
    prior_ind = eval(prior_ind);
end

% My prior_ind here is already those OK sight of lines that have MgII
prior.z_qsos  = PM_catalog.all_zqso;
prior.MgII_ind = prior_ind;
prior.z_MgII = PM_catalog.all_z_MgII1;

% filter out MgIIs from prior catalog corresponding to region of spectrum below
...
for all_quasar_ind =4
    ...
    % masking unwanted wavelengths, pixels without sigma_pixel, and CIV
    % emission region 
    ...
    if nnz(unmasked_ind)<min_num_pixels
        continue;
    end
...
%     % in read_spec_dr7.m
    ...
    this_p_MgII(1) = nnz(less_systems(:,1)>0 )/this_num_quasars;  % P(at least 1 MgII)
    for i=2:max_MgII
        this_p_MgII(i) = nnz(less_systems(:,i)>0)/nnz(less_systems(:,i-1)>0); % P(at least n MgII | at least n-1 MgII)
        if (this_p_MgII(i)==0 || isnan(this_p_MgII(i)))
            this_p_MgII(i) = 0.001;
        end
    end
    ...
    for i = 1:max_MgII
        ...
        log_priors_no_MgII(this_quasar_ind, i) = log(1 - this_p_MgII(i)); 
        fprintf(' ...     p(%i  MgIIs | z_QSO)       : %0.3f\n', i, this_p_MgII(i));
        fprintf(' ...     p(no MgII  | z_QSO)       : %0.3f\n', exp(log_priors_no_MgII(this_quasar_ind, i)) );
    end
    log_priors_MgII(this_quasar_ind,:) = log(this_p_MgII(:));

    % interpolate model onto given wavelengths
    this_mu = mu_interpolator( this_rest_wavelengths);
    this_M  =  M_interpolator({this_rest_wavelengths, 1:k});

    min_z_MgIIs(this_quasar_ind) = min_z_MgII(this_wavelengths, z_qso);
    max_z_MgIIs(this_quasar_ind) = max_z_MgII(z_qso, max_z_cut);

    sample_z_MgII = ...
        min_z_MgIIs(this_quasar_ind) +  ...
        (max_z_MgIIs(this_quasar_ind) - min_z_MgIIs(this_quasar_ind)) * offset_z_samples;
    ...
    for num_MgII=1:max_MgII
        ...
        if(num_MgII>1)
            if((p_MgII(this_quasar_ind, num_MgII-1)>p_MgIIL1(this_quasar_ind, num_MgII-1)) & ...
                (p_MgII(this_quasar_ind, num_MgII-1)>p_no_MgII(this_quasar_ind, num_MgII-1)))
                ind_not_remove = ind_not_remove  & ...
                    (abs(this_z_2796 - map_z_MgIIL2(this_quasar_ind, num_MgII-1))>kms_to_z(dv_mask)*(1+map_z_MgIIL2(this_quasar_ind, num_MgII-1))) & ...
                    (abs(this_z_2803 - map_z_MgIIL2(this_quasar_ind, num_MgII-1))>kms_to_z(dv_mask)*(1+map_z_MgIIL2(this_quasar_ind, num_MgII-1)));
            end

            if((p_MgIIL1(this_quasar_ind, num_MgII-1)>p_MgII(this_quasar_ind, num_MgII-1)) & ...
                        (p_MgIIL1(this_quasar_ind, num_MgII-1)>p_no_MgII(this_quasar_ind, num_MgII-1)))
                ind_not_remove = ind_not_remove  & ...
                (abs(this_z_2796 - map_z_MgIIL1(this_quasar_ind, num_MgII-1))>kms_to_z(dv_mask)*(1+map_z_MgIIL1(this_quasar_ind, num_MgII-1)));
            end

            if((p_no_MgII(this_quasar_ind, num_MgII-1)>=p_MgII(this_quasar_ind, num_MgII-1)) & ...
                (p_no_MgII(this_quasar_ind, num_MgII-1)>=p_MgIIL1(this_quasar_ind, num_MgII-1)))
        ...
        log_likelihoods_no_MgII(this_quasar_ind, num_MgII) = ...
        ...
        log_posteriors_no_MgII(this_quasar_ind, num_MgII) = ...
            log_priors_no_MgII(this_quasar_ind, num_MgII) + log_likelihoods_no_MgII(this_quasar_ind, num_MgII);
        ...
        parfor i = 1:num_MgII_samples
            ...
            sample_log_likelihoods_MgIIL2(this_quasar_ind, i, num_MgII) = ...
            ...
            sample_log_likelihoods_MgIIL1(this_quasar_ind, i) = ...
            log_mvnpdf_low_rank(this_flux(ind_not_remove),...
        ...
        max_log_likelihoodL1 = max(sample_log_likelihoods_MgIIL1(this_quasar_ind, :));
        sample_probabilitiesL1 = ...
            exp(sample_log_likelihoods_MgIIL1(this_quasar_ind, :) - ...
            max_log_likelihoodL1);
        log_likelihoods_MgIIL1(this_quasar_ind, num_MgII) = ...
            max_log_likelihoodL1 + log(mean(sample_probabilitiesL1)) ...
            - log(num_MgII_samples)*(num_MgII-1);

        log_posteriors_MgIIL1(this_quasar_ind, num_MgII) = ...
        log_priors_MgII(this_quasar_ind, num_MgII) + log_likelihoods_MgIIL1(this_quasar_ind, num_MgII);
        ...
        max_log_likelihoodL2 = max(sample_log_likelihoods_MgIIL2(this_quasar_ind, :, num_MgII));
        sample_probabilitiesL2 = ...
            exp(sample_log_likelihoods_MgIIL2(this_quasar_ind, :, num_MgII)  ... 
            - max_log_likelihoodL2);
        log_likelihoods_MgIIL2(this_quasar_ind, num_MgII) = ...
            max_log_likelihoodL2 + log(mean(sample_probabilitiesL2));



        log_posteriors_MgIIL2(this_quasar_ind, num_MgII) = ...
            log_priors_MgII(this_quasar_ind, num_MgII) + log_likelihoods_MgIIL2(this_quasar_ind, num_MgII);
        ...
        fprintf(' ... log p(MgII | D, z_QSO)        : %0.2f\n', ...
            log_posteriors_MgIIL2(this_quasar_ind, num_MgII));
        %  fprintf(' ... Num_MgII                      : %d\n ', ...
        %    Full_catalog.all_num_MgII_sys(this_quasar_ind))
        % fprintf('... FilterFlag                    : %d\n ', filter)
        [~, maxindL1] = max(sample_log_likelihoods_MgIIL1(this_quasar_ind, :), [], 'omitmissing');
        map_z_MgIIL1(this_quasar_ind, num_MgII )    = sample_z_MgII(maxindL1);        
        map_N_MgIIL1(this_quasar_ind, num_MgII)  = log_nMgII_samples(maxindL1);
        map_sigma_MgIIL1(this_quasar_ind, num_MgII)  = sigma_MgII_samples(maxindL1);
        fprintf('L1\nmap(N): %.2f, map(z_MgII): %.2f, map(b/1e5): %.2f\n',map_N_MgIIL1(this_quasar_ind, num_MgII),...
            map_z_MgIIL1(this_quasar_ind, num_MgII), map_sigma_MgIIL1(this_quasar_ind, num_MgII)/1e5);

        [~, maxindL2] = max(sample_log_likelihoods_MgIIL2(this_quasar_ind, :, num_MgII),[], 'omitmissing');
        map_z_MgIIL2(this_quasar_ind, num_MgII)    = sample_z_MgII(maxindL2);        
        map_N_MgIIL2(this_quasar_ind, num_MgII)  = log_nMgII_samples(maxindL2);
        map_sigma_MgIIL2(this_quasar_ind, num_MgII)  = sigma_MgII_samples(maxindL2);
        fprintf('L2\nmap(N): %.2f, map(z_MgII): %.2f, map(b/1e5): %.2f\n',...
        map_N_MgIIL2(this_quasar_ind, num_MgII), map_z_MgIIL2(this_quasar_ind, num_MgII),...
        map_sigma_MgIIL2(this_quasar_ind, num_MgII)/1e5);

        max_log_posteriors = max([log_posteriors_no_MgII(this_quasar_ind, num_MgII), log_posteriors_MgIIL1(this_quasar_ind, num_MgII), log_posteriors_MgIIL2(this_quasar_ind,num_MgII)], [], 2);

        model_posteriors = ...
                exp(bsxfun(@minus, ...           
                [log_posteriors_no_MgII(this_quasar_ind, num_MgII), log_posteriors_MgIIL1(this_quasar_ind, num_MgII), log_posteriors_MgIIL2(this_quasar_ind, num_MgII)], ...
                max_log_posteriors));
        model_posteriors = ...
        bsxfun(@times, model_posteriors, 1 ./ sum(model_posteriors, 2));

        p_no_MgII(this_quasar_ind, num_MgII) = model_posteriors(1);
        p_MgIIL1(this_quasar_ind, num_MgII)  = model_posteriors(2);
        p_MgII(this_quasar_ind, num_MgII)    = 1 - p_no_MgII(this_quasar_ind, num_MgII) -...
                                    p_MgIIL1(this_quasar_ind, num_MgII);

        MgII_pixel_ind1 = abs(this_wavelengths - (1+map_z_MgIIL2(this_quasar_ind, num_MgII))*1548.2040)<3;
        MgII_pixel_ind2 = abs(this_wavelengths - (1+map_z_MgIIL2(this_quasar_ind, num_MgII))*1550.7810)<3;
        num_pixel_MgII(this_quasar_ind, num_MgII, 1) = nnz(MgII_pixel_ind1);
        num_pixel_MgII(this_quasar_ind, num_MgII, 2) = nnz(MgII_pixel_ind2);
        fprintf('MgII pixels:[%d, %d]\n', num_pixel_MgII(this_quasar_ind, num_MgII, :)); 

        % % REW calculation 
        % aL1_fine = voigt_iP(padded_wavelengths_fine,...
        % map_z_MgIIL2(this_quasar_ind, num_MgII), ...
        % 10^map_N_MgIIL2(this_quasar_ind, num_MgII), 1,...
        % map_sigma_MgIIL2(this_quasar_ind, num_MgII), ...
        % padded_sigma_pixels_fine);
        % 
        % aL1 = Averager(aL1_fine, nAVG, lenW_unmasked);
        % REW_1548_dr16(this_quasar_ind, num_MgII) = trapz(this_unmasked_wavelengths, 1-aL1)/(1+z_qso);
        % 
        % fprintf('REW(%d,%d)=%e\n', this_quasar_ind, num_MgII, REW_1548_dr16(this_quasar_ind, num_MgII));

        % plotting
        if(plotting==1 && p_MgII(this_quasar_ind, num_MgII)>0.5) 

            max_log_posteriors = max([log_posteriors_no_MgII(this_quasar_ind, num_MgII),...
                                     log_posteriors_MgIIL1(this_quasar_ind, num_MgII),...
                                     log_posteriors_MgIIL2(this_quasar_ind,num_MgII)], [], 2);
            num_lines=1;
            absorptionL1_fine= voigt_iP(padded_wavelengths_fine,...
                            map_z_MgIIL1(this_quasar_ind, num_MgII), 10^map_N_MgIIL1(this_quasar_ind, num_MgII),...
                            num_lines, map_sigma_MgIIL1(this_quasar_ind, num_MgII), padded_sigma_pixels_fine);
            ...
            absorptionL2_fine= voigt_iP(padded_wavelengths_fine,...
                    map_z_MgIIL2(this_quasar_ind,  num_MgII), 10^map_N_MgIIL2(this_quasar_ind, num_MgII),...
                    num_lines, map_sigma_MgIIL2(this_quasar_ind, num_MgII), padded_sigma_pixels_fine);
            ...
            ttl = sprintf('ID:%s, zQSO:%.2f\n P(MgII)=%.2f, z_{MgII}=%.3f, P(S)=%.2f, SN=%.2f', ...
                all_QSO_ID_dr16{all_quasar_ind}, z_qso, p_MgII(this_quasar_ind, num_MgII), ...
                map_z_MgIIL2(this_quasar_ind, num_MgII),p_MgIIL1(this_quasar_ind, num_MgII), ... 
                median(this_flux./sqrt(this_noise_variance)));

            fid = sprintf('plt-DR16/ind-%d-MgII-%d.png', all_quasar_ind, num_MgII);
            if exist('plt-DR16')~=7
                mkdir('plt-DR16');
            end
            ind_zoomL2 = (abs( this_z_2796-map_z_MgIIL2(this_quasar_ind, num_MgII))<5*kms_to_z(dv_mask)*(1+z_qso));
            ind_zoomL1 = (abs( this_z_2796-map_z_MgIIL1(this_quasar_ind, num_MgII))<5*kms_to_z(dv_mask)*(1+z_qso));
            fprintf('min(Z):%.3f, max(Z):%.3f\n', min(sample_z_MgII), max(sample_z_MgII))
            indMAP = (abs(sigma_MgII_samples - map_sigma_MgIIL2(this_quasar_ind, num_MgII))<25e5);
            fprintf('min(Z(indMAP)):%.3f, max(Z(indMAP)):%.3f\n', min(sample_z_MgII(indMAP)), max(sample_z_MgII(indMAP)))
            pltQSO(this_flux, this_wavelengths, MgII_muL2, MgII_muL1, ttl, fid)
...
if saving==1
    variables_to_save = {'training_set_name', ...
        ...
        'map_z_MgIIL2', 'map_N_MgIIL2', 'map_sigma_MgIIL2', 'p_MgII', ...
        'map_z_MgIIL1', 'map_N_MgIIL1', 'map_sigma_MgIIL1'};
