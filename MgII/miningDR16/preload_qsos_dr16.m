...
% %filtering BALs
ind = (all_BAL_PROB >0.01);
filter_flags(ind) = bitset(filter_flags(ind), 1, true);

% coverage 
ind = (1310*(1+all_zqso_dr16)<3650) | (2850*(1+all_zqso_dr16)>10400); 
...
for i = 1:num_quasars
  ...
  %------dr16----------
  if (all_plate_dr16(i)>3006)
      filename = sprintf("Spectra/sas/dr16/eboss/spectro/redux/v5_13_0/spectra/lite/%i/spec-%i-%i-%04i.fits", ...
            all_plate_dr16(i),                                       ...
            all_plate_dr16(i),                                       ...
            all_mjd_dr16(i),                                         ...
            all_fiber_dr16(i));
      ...
      filename = sprintf("Spectra/sas/dr16/sdss/spectro/redux/103/%04i/spec-%04i-%i-%04i.fits", ...
            all_plate_dr16(i),                                       ...
            all_plate_dr16(i),                                       ...
            all_mjd_dr16(i),                                         ...
            all_fiber_dr16(i));
  ...
  %     % filter_flags(i) = bitset(filter_flags(i), 5, true); % file does not exist in eBoss
  %     fprintf('p:%04i--m:%0i--f:%0i', all_plate_dr16(i), all_mjd_dr16(i),  all_fiber_dr16(i));
  %     % break
  % % end
  % % else
...
variables_to_save = {'loading_min_lambda', 'loading_max_lambda', ...
                     'normalization_min_lambda_3', 'normalization_max_lambda_3', ...
