...
preqsos = load(sprintf('%s/preloaded_qsos_%s.mat', processed_directory(release), training_set_name));
...
if learning==1
    ...
    test_ind = (~ismember(all_QSO_ID, half_ID)) & (preqsos.filter_flags==0);
    prior_ind = ismember(all_QSO_ID, half_ID) & (preqsos.filter_flags==0);
    ...
    train_ind = (~ismember(all_QSO_ID, MgII_QSO_ID) & (preqsos.filter_flags==0) & ...
