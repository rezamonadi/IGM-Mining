...
% 
filename = sprintf('data/dr7/processed/processed_qsos_%s.mat', testing_set_name);   
...
variables_to_load = { 'training_set_name', ...
    ...
    'map_z_c4L2', 'map_N_c4L2', 'map_sigma_c4L2', 'p_c4', 'EW1_spline', ...
...
% EW_OK =   EW1_spline<=2*EW2_spline & EW1_spline>EW2_spline;
predicted_c4s =  (p_c4>tr);% & EW_OK;
% (EW1_spline>=EW2_spline) & (EW1_spline>0)...
                %   & (EW2_spline>0) & (EW1_spline<=2*EW2_spline) ;
% predicted_labels={0};
% true_labels={0};
...
exportgraphics(fig, sprintf('ROC-tr-%s-tst-%s.pdf', training_set_name, testing_set_name)...
                ,'ContentType','vector')
...
% figure();
% minEqW1 = linspace(min(EW1_spline), max(EW1_spline), 100);
% FP_EqW1=zeros(100,1);
% for i=1:100
%     FP_EqW1(i) = nnz(ind_FP==1 & EW1_spline<minEqW1(i));
% end
% plot(minEqW1, FP_EqW1, 'LineWidth',3)
% hold on 
% minEqW2 = linspace(min(EW2_spline), max(EW2_spline), 100);
% FP_EqW2=zeros(100,1);
% for i=1:100
%     FP_EqW2(i) = nnz(ind_FP==1 & EW2_spline<minEqW2(i));
% end
% plot(minEqW2, FP_EqW2, 'LineWidth',3)
% legend('EW1_spline', 'EW2_spline')
% save(gca, 'Eqw-FP.png')
...
fig4= figure();
h1 = histogram(EW1_spline(ind_FP & EW1_spline>0), 5);
v1 = h1.Values;
edgs = h1.BinEdges;
h2 = histogram(EW1_spline(EW1_spline>0), edgs);
v2 = h2.Values;
ratio = v1./v2;
centers = 0.5*(edgs(2:end) + edgs(1:end-1));
clf();
plot(centers, ratio, '-*')
set(get(gca, 'XLabel'), 'String', 'EW1Spline');
set(get(gca, 'YLabel'), 'String', 'Ratio(FP)');
exportgraphics(fig4,'FP-EW1Spline.png', 'Resolution',800)    


fig5= figure();
h1 = histogram(EW2_spline(ind_FP & EW2_spline>0), 5);
v1 = h1.Values;
edgs = h1.BinEdges;
h2 = histogram(EW2_spline(EW2_spline>0), edgs);
v2 = h2.Values;
ratio = v1./v2;
centers = 0.5*(edgs(2:end) + edgs(1:end-1));
clf();
plot(centers, ratio, '-*')
set(get(gca, 'XLabel'), 'String', 'EW2Spline');
set(get(gca, 'YLabel'), 'String', 'Ratio(FP)');
exportgraphics(fig5,'FP-EW2Spline.png', 'Resolution',800)  



fig6= figure();
DR = EW1_spline./EW2_spline;
h1 = histogram(DR(ind_FP & EW_OK), 5);
v1 = h1.Values;
edgs = h1.BinEdges;
h2 = histogram(DR(EW_OK), edgs);
v2 = h2.Values;
ratio = v1./v2;
centers = 0.5*(edgs(2:end) + edgs(1:end-1));
clf();
plot(centers, ratio, '-*')
set(get(gca, 'XLabel'), 'String', 'DR');
set(get(gca, 'YLabel'), 'String', 'Ratio(FP)');
exportgraphics(fig6,'FP-DRSpline.png', 'Resolution',800)  
