...
class DLACatalogue(object):
    ...
    * :attr:`lowzcut` (boolean): remove spectra closer to the DLA than this
    ...
    """
    def __init__(self, processed_file = "pltData.mat", sample_file = "../data/dr7/processed/civ_samples_N_1288_1500_Sigma_500000_4000000_num_30000_alpha_90.mat",
            raw_file = "../data/dr12/preloaded_qsos_C13_full.mat", snrs_file = "snrs_qsos_dr12.mat",
            snr = -2, lowzcut=False, second=False, sub_dla=False, occams_razor=1):
        ...
        self._z_min = self.filehandle["all_min_z_c4s"][0]
        self._z_max = self.filehandle["all_max_z_c4s"][0]
        ...
        print(np.size(ff["snrs"][:]))
        print(np.sum(self.filehandle["test_ind"]))
        try:
...
def find_snr(nspec, real_index, raw_file, zmin, zmax, civ_1548_wavelength):
    ...
    # ipix = np.where(np.logical_and(wavelengths > civ_1548_wavelength*(1+ zmin), wavelengths < civ_1548_wavelength*(1+zmax)))
    ipix = np.where(wavelengths > civ_1548_wavelength*(1+zmax))
    ...
    hh.close()
    return 1/np.median(np.sqrt(noise_var.T)/np.abs(flux.T))
...
def find_pixel_snr(nspec,real_index, raw_file, zmin, zmax, loading_min_lambda, loading_max_lambda):
    """Find pixels where the absolute value of the noise is below thresh a particular value.
    So we want pixels with: all_noise_variance/all_normalizers^2 < thresh^2
    where all_noise_variance is the noise and defined in preloaded_qsos."""
    nspec_real = real_index[nspec]
    hh = h5py.File(raw_file,'r')
    wavelengths = hh[hh["all_wavelengths"][0][nspec_real]][0]
    ipix = np.where(np.logical_and(wavelengths > loading_min_lambda*(1+ zmin), wavelengths < loading_max_lambda*(1+zmax)))
    flux = np.array(hh[hh["all_flux"][0][nspec_real]][0])[ipix]
    noise_var = np.array(hh[hh["all_noise_variance"][0][nspec_real]][0])[ipix]
    try:
        norm = hh["all_normalizers"][0][nspec_real]
...
def compute_all_snrs(*, raw_file="../data/dr12/preloaded_qsos_C13_full.mat", processed_file="pltData.mat", save_file="snrs_qsos_dr12.mat"):
    ...
    min_z_c4s = np.array(ff["all_min_z_c4s"][0])
    max_z_c4s = np.array(ff["all_max_z_c4s"][0])
   
    ff.close()
    snrs = np.array([find_snr(nn, real_index, raw_file, min_z_c4s[nn], max_z_c4s[nn], civ_1548_wavelength) for nn in tqdm(range(np.size(real_index)))])
